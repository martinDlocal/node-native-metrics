...
var QUIET = !!process.env.PRE_BUILD_QUIET


exports.load = load

if (require.main === module) {
  executeCli(process.argv[2], process.argv[3])
}
...
function execGyp(args, cb) {
  ...
  var child = cp.spawn('node-gyp', args, opts)
...
function install(target, cb) {
  ...
  function doDownload() {
    if (process.env.PRE_BUILD_NO_DOWNLOAD) {
