...
function execGyp(args, cb) {
  ...
  console.log('> ' + gyp + ' ' + args.join(' ')) // eslint-disable-line no-console

  var child = cp.spawn(gyp, args, spawnOpts)
  child.on('error', function onGypError(err) {
    cb(new Error('Failed to execute ' + gyp + ' ' + args.join(' ') + ': ' + err))
  })
  child.on('close', function onGypClose(code) {
    if (code !== 0) {
      cb(new Error('Failed to execute ' + gyp + ' ' + args.join(' ') + ': code ' + code))
    } else {
      cb(null)
    }
  })
